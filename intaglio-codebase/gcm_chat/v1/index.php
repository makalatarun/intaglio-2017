...
$app->get('/chat_rooms/:id', function($chat_room_id) {
    ...
    $response["error"] = false;
    $response['chatroom_messages'] = array();
    ...
    foreach ($result as $a){
        ...
        if ($i == 0) {
            $tmp = array();
            $tmp["chat_room_id"] = $a["chat_room_id"];
            $tmp["name"] = $a["name"];
            $tmp["created_at"] = $a["chat_room_created_at"];
        ...
        if ($a['user_id'] != NULL) {
            // message node
            $cmt = array();
            $cmt["message"] = $a["message"];
            $cmt["message_id"] = $a["message_id"];
            $cmt["created_at"] = $a["created_at"];
            ...
            array_push($response['chatroom_messages'], $cmt);
        }
    }
